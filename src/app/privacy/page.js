      ...
      <main className="max-w-4xl mx-auto px-6 md:px-12 py-16">
        <div className={`max-w-3xl mx-auto ${theme.card} rounded-3xl p-8 md:p-12`}>
          ...
          <div className="prose prose-lg max-w-none space-y-8">
            <section>
              <h2 className={`text-2xl font-bold ${theme.heading} mb-4`}>Information We Collect</h2>
              <p className={`leading-relaxed ${theme.text} mb-4`}>
                We collect information you provide directly to us, such as when you create an account, book a demo, or contact us for support.
              </p>
              <ul className="space-y-2 ml-6">
                <li className={`flex items-start gap-2 ${theme.text}`}>
                  <CheckCircle size={16} className="mt-1 shrink-0 text-green-500" />
                  <span>Name and contact information</span>
                </li>
                <li className={`flex items-start gap-2 ${theme.text}`}>
                  <CheckCircle size={16} className="mt-1 shrink-0 text-green-500" />
                  <span>Company details and business requirements</span>
                </li>
                <li className={`flex items-start gap-2 ${theme.text}`}>
                  <CheckCircle size={16} className="mt-1 shrink-0 text-green-500" />
            ...
            <section>
              <h2 className={`text-2xl font-bold ${theme.heading} mb-4`}>How We Use Your Information</h2>
              <p className={`leading-relaxed ${theme.text} mb-4`}>
                Your information helps us provide better service, improve our platform, and communicate with you about updates and support.
              </p>
              <div className={`${theme.card} rounded-xl p-6 ${isDark ? 'bg-violet-500/10 border-violet-500/20' : 'bg-blue-50 border-blue-200'}`}>
                <h3 className={`font-bold ${theme.heading} mb-2`}>Our Commitment</h3>
                <p className={theme.text}>
                  We never sell your personal information to third parties. Your data is encrypted and stored securely.
                </p>
              </div>
            </section>

            <section>
              <h2 className={`text-2xl font-bold ${theme.heading} mb-4`}>Data Security</h2>
              <div className="flex items-start gap-4">
                <Shield size={24} className={`mt-1 ${theme.accent}`} />
                <div>
                  <p className={`leading-relaxed ${theme.text} mb-2`}>
                    We implement industry-standard security measures including:
                  </p>
                  <ul className="space-y-1 ml-4">
                    <li className={`text-sm ${theme.text}`}>• End-to-end encryption</li>
                    <li className={`text-sm ${theme.text}`}>• Secure data centers</li>
                    <li className={`text-sm ${theme.text}`}>• Regular security audits</li>
                    <li className={`text-sm ${theme.text}`}>• GDPR compliance</li>
                  </ul>
                </div>
              </div>
            </section>
            ...
            <section className={`mt-8 p-6 rounded-2xl ${isDark ? 'bg-violet-500/10 border-violet-500/20' : 'bg-blue-50 border-blue-200'}`}>
              <h3 className={`text-lg font-bold ${theme.heading} mb-2`}>Contact Us</h3>
              <p className={`mb-4 ${theme.text}`}>
                If you have questions about this Privacy Policy, please contact us at privacy@autoship.com
              </p>
            </section>
          </div>
        </div>
